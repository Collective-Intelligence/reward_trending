...
keywordlist = [["type","account"],["gp",0],["ad-token-perm",0],["token-upvote-perm",0],["ad-token-temp",0],
               ...
               ["steem-gp-ratio",0.33],["groups",[["CI",1]]],["rating_curation",0],["adp_tok",10], ["version",0], ["changes",[]],["old",0]]
...
def get_account_info(account,active_key,our_account = "anarchyhasnogods", our_memo_account = "space-pictures",node ="wss://steemd-int.steemit.com", position = False):
    ...
    if position:
        return_info = main.retrieve([["account", account], ["type", "account"]], account=our_account,
                                    sent_to=our_memo_account, node=node, position=position)
    else:
...
def update_account(account, our_sending_account, our_memo_account, changes, active_key,node, account_info = []):
    ...
    print("UPDATING ACC")
    print(account,our_sending_account,our_memo_account, changes)
    if account_info == []:
    ...
    for i in changes:
        ...
        if i[0] == "vote":
            ...
            info_dict[i[0]] = i[1]
    try:
        info_dict["changes"] = changes
        info_dict["version"] += 1
        info_dict["old"] = old_memo_pos
    except:
        pass
...
def pay_account(size,our_account,our_memo_account,node,active_key,info, not_owed = False):
